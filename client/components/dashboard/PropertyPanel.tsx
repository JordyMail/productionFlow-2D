...
const filterOperatorNodes = (
  nodes: Node<NodeData>[],
  predicate: (node: Node<OperatorData>) => boolean,
): Node<OperatorData>[] => {
  return nodes.filter(
    (node): node is Node<OperatorData> =>
...
const PropertyPanel = () => {
  ...
  const getHandleStatus = (position: HandlePosition): boolean => {
    ...
    const handles = (selectedNode.data as any).handles as
      | HandleConfig
      | undefined;
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-amber-600">
                  This {isOperator ? "operator" : "machine"} cannot connect to
                  any node. Enable at least one handle to allow connections.
                </p>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="w-full h-full bg-slate-50 border-l border-slate-200 flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-xs text-slate-400 max-w-[200px]">
          Click on any machine or operator in the canvas to view and edit its
          properties.
        </p>
      </div>
    );
  ...
  const isMachine =
    selectedNode.type === "machineNode" ||
    selectedNode.type === "shapeMachineNode";
  const isOperator = selectedNode.type === "operatorNode";
  ...
  const handleOperatorChange = (field: "id" | "process", value: number) => {
    ...
    if (field === "process") {
      ...
      if (isDuplicate) {
        alert(
          `Process ${value} is already used by another operator with ID ${operatorData.id}`,
        );
        return;
      }
    }
  ...
  const handleDelete = () => {
    if (
      confirm(
        `Are you sure you want to delete this ${isMachine ? "machine" : "operator"}?`,
      )
    ) {
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      const currentChair = operatorData.chairDesign || {
                        enabled: true,
                        chairColor: operatorData.color || "#afbfe4",
                        showIdInChair: true,
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          type="color"
                          value={
                            operatorData.chairDesign?.chairColor ||
                            operatorData.color ||
                            "#afbfe4"
                          }
                          onChange={(e) => {
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            operatorData.chairDesign?.showIdInChair !== false
                          }
                          onCheckedChange={(checked) => {
                            const currentChair = operatorData.chairDesign || {
                              enabled: true,
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            operatorData.chairDesign?.showProcessInChair !==
                            false
                          }
                          onCheckedChange={(checked) => {
                            const currentChair = operatorData.chairDesign || {
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              if (!isNaN(val) && val >= 40 && val <= 200) {
                                const currentChair =
                                  operatorData.chairDesign || {};
                                updateNodeData(selectedNode.id, {
                                  chairDesign: {
                                    ...currentChair,
                                    enabled: currentChair.enabled !== false,
                                    chairWidth: val,
                                  },
                                });
                              }
                      ...
                      <div className="flex items-center justify-center">
                        ...
                        >
                          ...
                          <rect
                            ...
                            height="37"
                            rx="3"
                            fill={`${operatorData.chairDesign?.chairColor || operatorData.color || "#afbfe4"}cc`}
                          />
                          <rect
                            x="20"
                            y="48"
            ...
            <div className="mt-4 p-4 bg-purple-50 rounded-xl border border-purple-200">
              ...
              {otherOperators.length === 0 ? (
                <p className="text-xs text-purple-600">
                  No other operators with ID {operatorData.id}. Add more to
                  create connections.
                </p>
              ) : (
                <div>
                  <p className="text-xs text-purple-600 mb-2">
                    Connected to {otherOperators.length} operator(s) with same
                    ID:
                  </p>
                    ...
                    {otherOperators
                      ...
                      .map((op) => (
                        <div
                          ...
                          style={{
                            borderLeftColor: op.data.color || "#afbfe4",
                            borderLeftWidth: "3px",
                          }}
                        >
  ...
  if (isMachine && isMachineData(data)) {
    ...
    const currentTemplate = currentTemplateId
      ...
      : null;
    const frameRotation =
      (machineData as any).frameRotation || currentTemplate?.frameRotation || 0;
                  ...
                  <div className="mt-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-slate-400">
                          {currentTemplate.frameType === "rectangle" &&
                            "Square 1x1"}
                          {currentTemplate.frameType === "rectangle2x1" &&
                            "Rectangle"}
                          {currentTemplate.frameType === "circle" && "Circle"}
                          {currentTemplate.frameType === "triangle" &&
                            "Triangle"}
                          {" • "}
                          {frameRotation}°
                        </p>
